...
class kmeans:
	...
	def kmeansfwd(self,data):
		#collects the mean squared error that is calculated and then take the average of those for each K value
		meanerror = []
		avgerror = 0
		...
		for j in range(self.k-1):
			...
			meanerror.append(distances)

		#make a new list that contains the average of the averages per segment
		overallavgs = []
		
		#for each list in meanerror
		for item in meanerror:
			smallsum = 0.0
			smallavg = 0.0
			...
			for sublist in item:
				subsum = 0.0
				subavg = 0.0
				#now for each number in the sublist
				for num in sublist:
					subsum += num
				subavg = subsum/(len(sublist))
				smallsum += subavg	
			smallavg = smallsum/(len(sublist))
			overallavgs.append(smallavg)

		#calculate so you can compare for each iteration
		overallsum = 0
		for mean in overallavgs:
			overallsum += mean
		avgerror = overallsum/len(overallavgs)
